...
class feed_provider config (distro:Distro.distribution) =
  object (self : #Feed_provider.feed_provider)
    ...
    val mutable distro_cache : (Feed.distro_implementation Feed.ImplementationMap.t * Feed.feed_overrides) FeedMap.t = FeedMap.empty
    ...
    method get_distro_impls feed : (Feed.distro_implementation Feed.ImplementationMap.t * Feed.feed_overrides) =
