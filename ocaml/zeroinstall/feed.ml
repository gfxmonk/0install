    ...
    if machine = Some "src" then begin match StringMap.find "compile" !s.commands with
      | Some command ->
        ...
        command.command_qdom |> Compile.iter ~name:"implementation" (fun child ->
          (* synthesize minimal attributes from the source *)
          let synthetic_attrs = AttrMap.empty
            |> AttrMap.add_no_ns "id" (get_attr_ex "id" impl)
            |> AttrMap.add_no_ns "version" (get_attr_ex "version" impl)
          in
          ...
          let synthetic_impl = ZI.make
            ~source_hint:child
            ~attrs:(synthetic_attrs |> AttrMap.add_all child.Q.attrs)
            ~child_nodes:child.Q.child_nodes
            "implementation" in
          let compiled_state = root_state in
