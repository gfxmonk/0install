...
let autocompile options sels reason =
  ...
  let compile_sels = Generic_select.handle options [`Source (Some false); `NotBefore "1.3-post"] "http://0install.net/2006/interfaces/0compile.xml" `Select_for_run in
...
let handle options flags args =
  ...
  | arg :: run_args -> (
    ...
    let sels = if Zeroinstall.Selections.has_source_impls sels
      then begin
        autocompile options sels arg;
        (* TODO: pin exact implementation IDs here, for efficiency? *)
        do_select [`Source (Some false)]
      end else sels in
